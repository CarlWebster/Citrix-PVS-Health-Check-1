...
If(!(Check-NeededPSSnapins "McliPSSnapIn"))
{
	...
	If(Test-Path "$PFiles\Citrix\Provisioning Services Console\McliPSSnapIn.dll" -EA 0)
	{
		...
		{
			&$installutil "$PFiles\Citrix\Provisioning Services Console\McliPSSnapIn.dll"
		
			If(!$?)
			{
				Write-Error "Unable to register the 32-bit V2 PowerShell Snap-in."
			}
		}
	...
	If(Test-Path "$PFiles\Citrix\Provisioning Services Console\McliPSSnapIn.dll" -EA 0)
	{
		...
		If(Test-Path $installutil -EA 0)
		{
			...
			If(!$?)
			{
				Write-Error "Unable to register the 32-bit V4 PowerShell Snap-in."
			}
		}
